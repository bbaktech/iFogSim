...
public class ModulePlacementEdgewardNew extends ModulePlacement{
	...
	public ModulePlacementEdgewardNew(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, 
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		for(AppEdge edge : getApplication().getEdges()){
			if(edge.isPeriodic()){
				appEdgeToRate.put(edge, (double) (1/100));
			}
		}
			...
			while(modulesToPlace.size() > 0){ // Loop runs until all modules in modulesToPlace are deployed in the path
				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						...
						}  //Third  Change Part one
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > 2000){
